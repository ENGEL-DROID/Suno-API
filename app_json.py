...
def generate_songs_from_prompts(prompts_file):
    ...
    for prompt_data in prompts:
        ...
        for variation in variations:
            ...
            tags = variation['tags']
            title = prompt  # Usar el prompt como título de la canción
            ...
            for tag in tags:
                try:
                    songs = client.generate(
                        ...
                        is_custom=False,
                        tags=tag,
                        title=title,  # Nombre de la canción con prompt
                        wait_audio=True
                    )

                    # Descargar y guardar las canciones y letras
                    for index, song in enumerate(songs, start=1):
                        try:
                            ...
                            new_audio_filename = f"{title} - {tag} - {index}.mp3"
                            new_audio_file_path = os.path.join(category_folder, new_audio_filename)

                            # Construir el nombre de archivo para guardar la información
                            info_filename = f"{title} - {tag} - {index}.json"
                            info_file_path = os.path.join(category_folder, info_filename)
                            ...
                            if song_info['lyrics']:
                                ...
                                lyrics_filename = f"{title} - {tag} - {index}.txt"
                                lyrics_file_path = os.path.join(category_folder, lyrics_filename)
